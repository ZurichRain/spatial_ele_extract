...
class ori_data(update_pro_class):
    def __init__(self) -> None:
        super().__init__()
        self.id2obj=dict()
        self.seqlis=[] #切分后的seq列表
        self.seqoriids=[] #对应原始文档中的字符位置
        self.seqtoklis=[] #token后的seq列表
        self.seqtokidslis=[] #tokenids 
        # self.seq_tok_lab=[] #token 之后的lab(空间元素的标签)
        # self.seq_sr_tok_lab=[] # 空间角色的标签
        # self.doc_link_lis=[] #空间关系训练数据
        # self.doc_link_lab=[] #空间关系标签
        self.all_link_seqs=[]
        self.all_ele_labels=[]
        self.all_qs_role_labels=[]
